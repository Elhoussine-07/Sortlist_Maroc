...
class AgencyReview(Document):
    ...
    def before_insert(self):
        # Vérifier que le client existe. AgencyReview.client est un Link vers
        # "User" (l'email de session), contrairement à Project.client qui est
        # le NOM du ClientProfile — ne pas confondre les deux.
        if not self.client:
        ...
        # Vérifier que le client est le propriétaire du projet. Project.client
        # est le NOM du ClientProfile ; il faut le résoudre vers l'email
        # (self.client) avant de comparer.
    ...
    def on_update(self):
        ...
        # qui empêchait la branche "Approved" ci-dessous de jamais se
        # déclencher. `get_doc_before_save()` donne le vrai état d'avant
        # modification.
